...
Circle::Circle(float x, float y, float rad, bool on) : StimArea(x, y, on)
{
    m_rad = rad;
}
...
Rect::Rect(float x, float y, float w, float h, bool on) : StimArea(x, y, on)
{
    m_w = w;
    m_h = h;
}
...
void Rect::set(float x, float y, float w, float h, bool on)
{
    m_cx = x;
    m_cy = y;
    m_w = w;
...
bool Rect::isPositionIn(float x, float y)
{
...
float Rect::distanceFromCenter(float x, float y){
    return std::abs(x - m_cx) + std::abs(y - m_cy);
}

String Rect::returnType()
{
    return String("rect");
}
