...
FPGAchannelList::FPGAchannelList(GenericProcessor* proc_, Viewport* p, FPGAcanvas* c) : chainUpdate(false), viewport(p), canvas(c)
{
    ...
    RHS2000Editor* e = static_cast<RHS2000Editor*>(proc->getEditor());
...
{
    RHS2000Editor* p = (RHS2000Editor*)proc->getEditor();
...
void FPGAchannelList::update()
{
   // const int columnWidth = 330;
    ...
    RHS2000Thread* thread = (RHS2000Thread*)proc->getThread();
...
void FPGAchannelList::comboBoxChanged(ComboBox* b)
{
    if (b == numberingScheme)
    {
        ...
        RHS2000Thread* thread = (RHS2000Thread*)p->getThread();
...
void FPGAcanvas::update()
{
    ...
    channelList->update();
    if (static_cast<RHS2000Thread*>(processor->getThread())->isAcquisitionActive())
	{
...
RHS2000Editor::RHS2000Editor(GenericProcessor* parentNode,
                             RHS2000Thread* board_,
...
RHS2000Editor::~RHS2000Editor()
{

}

void RHS2000Editor::scanPorts()
{
    rescanButton->triggerClick();
}

void RHS2000Editor::measureImpedance()
{
	impedanceData->valid = false;
	board->runImpedanceTest(impedanceData);
}

void RHS2000Editor::handleAsyncUpdate()
{
...
void RHS2000Editor::setSaveImpedance(bool en)
{
    saveImpedances = en;
}

void RHS2000Editor::setAutoMeasureImpedance(bool en)
{
    measureWhenRecording = en;
}

bool RHS2000Editor::getSaveImpedance()
{
    return saveImpedances;
}

bool RHS2000Editor::getAutoMeasureImpedance()
{
    return measureWhenRecording;
}

void RHS2000Editor::comboBoxChanged(ComboBox* comboBox)
{
...
void RHS2000Editor::buttonEvent(Button* button)
{
...
void RHS2000Editor::channelChanged (int channel, bool /*newState*/)
{
...
void RHS2000Editor::startAcquisition()
{
...
void RHS2000Editor::stopAcquisition()
{
...
void RHS2000Editor::saveCustomParameters(XmlElement* xml)
{
...
void RHS2000Editor::loadCustomParameters(XmlElement* xml)
{
...
Visualizer* RHS2000Editor::createNewCanvas()
{
...
BandwidthInterface::BandwidthInterface(RHS2000Thread* board_,
                                       RHS2000Editor* editor_) :
    board(board_), editor(editor_)
{
    ...
    upperBandwidthSelection = new Label("UpperBandwidth",lastHighCutString); // this is currently set in RHS2000Thread, the cleaner would be to set it here again
...
SampleRateInterface::SampleRateInterface(RHS2000Thread* board_,
                                         RHS2000Editor* editor_) :
    board(board_), editor(editor_)
{
...
HeadstageOptionsInterface::HeadstageOptionsInterface(RHS2000Thread* board_,
                                                     RHS2000Editor* editor_,
                                                     int hsNum) :
...
AudioInterface::AudioInterface(RHS2000Thread* board_,
                               RHS2000Editor* editor_) :
    board(board_), editor(editor_)
{
    ...
    noiseSlicerLevelSelection = new Label("Noise Slicer",lastNoiseSlicerString); // this is currently set in RHS2000Thread, the cleaner would be to set it here again
...
DSPInterface::DSPInterface(RHS2000Thread* board_,
                           RHS2000Editor* editor_) :
    board(board_), editor(editor_)
{
    name = "DSP";
